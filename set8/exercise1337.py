...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    the_answer = f"🔥{'🔥'.join(list(input_string.upper()))}🔥"
    return the_answer
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet :
        ...
        pet_count[letter] = len(pets_list)

    most_popular_letter = max(pet_count, key=pet_count.get)
...
def random_filler_text(number_of_words=200) -> str:
    ...
    words = []
    word_length = list(my_dict.keys())
    for i in range(number_of_words):
        random_length = random.choice(word_length)
        random_text = random.choice(my_dict[random_length])
        words.append(random_text)
